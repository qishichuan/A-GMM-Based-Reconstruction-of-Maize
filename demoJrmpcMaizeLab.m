...
fname = arrayfun(@(idx) sprintf('C:/Users/hzyll/OneDrive/Desktop/Research/LiuLu/Data/2021-08-26-11-08-03_Velodyne-VLP-16-Data/%d.txt',idx),idx,'uniformoutput',false);

fprintf('Maize data loading from ''C:/Users/hzyll/OneDrive/Desktop/Research/LiuLu/Data/2021-08-26-11-08-03_Velodyne-VLP-16-Data/''.\n');
...
df = 3; % df=1 means no downsampling
...
Xin = Xin*20; % make them have the same order with points (it helps the convergence)
...
% cellfun(@(TV) scatter3(TV(1,:),TV(2,:),TV(3,:),5,'filled'),V(1:3:M));
cellfun(@(TV,I) scatter3(TV(1,:),TV(2,:),TV(3,:),3,I,'filled'),TV(1:M),I(1:M));

set(1,'position',get(1,'position')+[0 0 0 0]);
...
cellfun(@(TVref,Iref) scatter3(TVref(1,:),TVref(2,:),TVref(3,:),3,Iref,'filled'),TVrefined,Irefined);

set(2,'position',get(2,'position')+[0 0 0 0]);
...
figure
TVpoint = cell2mat(TVrefined')';
TVind = cell2mat(Irefined);
Ind = TVpoint(:,1)>-100;%aa(:,1)>0.8 & aa(:,1)<1.2;
aa = pointCloud(TVpoint(Ind,:),'Color',TVind(Ind,:));
pcshow(aa)

figure
ind = TVpoint(:,1)>-0.7 & TVpoint(:,1)<0 & TVpoint(:,2)>0.5 &TVpoint(:,2)<1.5;
bb = TVpoint(ind,:);
iii = TVind(ind,:);
bb = pointCloud(bb);%,'Color',iii);
pcshow(bb)
